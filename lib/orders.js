...
export async function createOrder(input, { actor, source = 'manual' } = {}) {
  ...
  const orderDate = parseTime(input.order_date, 'Order date') ?? null;
  const orderValue = parseMoney(input.order_value) ?? null;
  ...
  return inTransaction(async (client) => {
    ...
    if (note) await logEvent(client, id, 'note_added', actor, { note });
    return id;
  });
}
