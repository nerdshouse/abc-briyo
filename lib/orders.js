...
const ORDER_COLUMNS = `
  ...
  (SELECT count(*)::int FROM order_documents d WHERE d.order_id = o.id AND d.removed_at IS NULL
...
async function insertOrder(client, f, { actor, source }) {
  ...
  const { rows } = await client.query(
    ...
    [f.channel, f.sourceOrderId, f.orderDate, f.orderValue, f.currency, f.customerName, f.customerPhone,
      f.customerEmail, f.paymentMethod, f.paymentStatus, f.fulfillmentType, source, actor]);
