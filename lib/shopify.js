 ...
 */
import { loadToken } from './shopify-oauth.js';

const isPlaceholder = (v) => !v || /^(shpat_)?x+$/i.test(String(v).trim())
  || /^(replace|your[-_]?|paste|<)/i.test(String(v).trim());

/**
 * The legacy permanent token, but only if it is a real one. A left-over
 * placeholder must not win over working client credentials — otherwise every
 * call 401s while the app insists it is configured.
 */
const staticToken = () => {
...
export const authMode = () =>
  staticToken() ? 'static token (legacy custom app)' : 'oauth, falling back to client credentials';
...
async function fetchClientCredentialsToken() {
  ...
  if (!res.ok) {
    // Shopify answers this endpoint with an HTML error page, so the useful part
    // is the <title>. Name the two failures we actually hit rather than making
    // the next person read a page of markup.
    const title = (text.match(/<title>([^<]*)<\/title>/i)?.[1] || '').trim();
    let hint = `Check SHOPIFY_CLIENT_ID / SHOPIFY_CLIENT_SECRET, and that the app `
      + `and store are in the same Shopify organization.`;
    if (/app_not_installed/i.test(text)) {
      // Almost always this means the store is outside the app's organization,
      // which is the one thing the client credentials grant cannot do.
      hint = `Shopify reports the app as not installed on ${domain}. If that store is `
        + `a live store rather than a Dev Dashboard dev store, the client credentials `
        + `grant cannot reach it at all — connect it once via /auth/shopify/install, `
        + `which uses the authorization code grant instead.`;
    } else if (/invalid_client|unauthorized_client/i.test(text)) {
      hint = `Shopify rejected the client ID/secret. Re-copy both from the Dev `
        + `Dashboard (app -> App settings); the secret is shown in full only there.`;
    }
    throw new Error(
      `Could not get a Shopify token (HTTP ${res.status}${title ? `: ${title}` : ''}). ${hint}`,
    );
  }
...
export async function getAccessToken({ force = false } = {}) {
  ...
  // An OAuth token is offline and does not expire, so `force` cannot improve it
  // — a 401 on one means the app was uninstalled or its scopes changed, and the
  // fix is to walk through /auth/shopify/install again, not to refetch.
  const stored = await loadToken(process.env.SHOPIFY_STORE_DOMAIN).catch(() => null);
...
function endpoint() {
  const domain = process.env.SHOPIFY_STORE_DOMAIN;
  // A retired version 404s with no hint of why, so an EOL setting is worse than
  // none: ignore it (apiVersionWarning() already says so at boot) and use the
  // current default rather than failing every call.
  const configured = process.env.SHOPIFY_API_VERSION;
  const version = configured && !apiVersionWarning() ? configured : DEFAULT_API_VERSION;
