...
export const authMode = () =>
  staticToken() ? 'static token (legacy custom app)' : 'client credentials (Dev Dashboard app)';
...
async function fetchClientCredentialsToken() {
  ...
  if (!res.ok) {
    ...
    if (/app_not_installed/i.test(text)) {
      hint = `The app is not installed on ${domain}. Install it from the Dev Dashboard `
        + `(app -> Overview -> Install), and make sure read_orders is in its access `
        + `scopes first — scopes are fixed at install time.`;
