...
import {
  ...
  orderDocuments, getDocument, orderEvents, createOrder, updateOrder, addOrderNote,
  addDocument, removeDocument, ORDER_STATUSES, SHIPMENT_STATUSES, PAYMENT_STATUSES,
  DOCUMENT_TYPES, LOGISTICS_VIEWS,
} from './orders.js';
import { storage, validateDocument, newStorageKey, MAX_DOCUMENT_BYTES } from './storage.js';
...
const tz = () => process.env.BOARD_TIMEZONE || process.env.BOARD_TZ || 'Asia/Kolkata';
...
const send = (res, err) => {
  if (err.status && err.status < 500) {
    return res.status(err.status).json({ ok: false, error: err.message, existingId: err.existingId, conflict: err.conflict });
  }
  console.error(err);
...
router.get('/meta', async (req, res) => {
  try {
    ...
    res.json({
      ...
      storage: { driver: store.name, durable: store.durable },
      timezone: tz(),
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [documents, events] = await Promise.all([orderDocuments(id), orderEvents(id)]);
    res.json({ ok: true, order, documents, events });
  } catch (err) { send(res, err); }
});
...
// Upload: the raw file is the body; its name and kind travel in headers and
// the query. No multipart parser needed, and nothing binary touches Postgres.
router.post('/:id/documents',
  ...
  async (req, res) => {
    ...
    try {
      ...
      const filename = decodeURIComponent(String(req.get('x-filename') || '')).replace(/[\\/]/g, '_').trim();
      const documentType = String(req.query.type || '');
      if (!DOCUMENT_TYPES.includes(documentType)) {
        return res.status(400).json({ ok: false, error: 'Choose what kind of document this is.' });
      }
      const buffer = Buffer.isBuffer(req.body) ? req.body : Buffer.alloc(0);
      const check = validateDocument(filename, buffer);
      if (!check.ok) return res.status(400).json({ ok: false, error: check.error });

      key = newStorageKey(id, check.ext);
      ...
      key = null; // committed
      res.status(201).json({ ok: true, documentId: docId });
    } catch (err) {
      // The row never landed, so the file would be an orphan.
...
router.get('/:id/documents/:docId', async (req, res) => {
  try {
    ...
    let bytes;
    try { bytes = await storage().get(doc.storage_path); } catch {
      return res.status(410).json({
        ok: false,
        error: 'The file is no longer in storage. Local storage is wiped on redeploy — upload it again.',
      });
    }
...
courierRouter.post('/', adminOnly, async (req, res) => {
  try { res.status(201).json({ ok: true, courier: await saveCourier(req.body ?? {}) }); } catch (err) {
...
courierRouter.patch('/:id', adminOnly, async (req, res) => {
  try {
    const courier = await saveCourier({ ...req.body, id: idOf(req.params.id) });
