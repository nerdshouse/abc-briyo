...
export async function seedAllowedUsers() {
  ...
  for (const [phone, name] of fromEnv) {
    await getPool().query(
      `INSERT INTO allowed_users (phone, name, is_admin, added_by)
       VALUES ($1, $2, $3, 'seed:ALLOWED_PHONES')
       ON CONFLICT (phone) DO NOTHING`, [phone, name, admins.has(phone)]);
  }
  return { seeded: fromEnv.size, source: fromEnv.size ? 'seeded from ALLOWED_PHONES' : 'empty' };
}

/**
 * Numbers that are admins regardless of what the table says.
 *
 * This is the anti-lockout lever: member management lives in the database, so
 * without a way to grant admin from outside it, a bad edit could leave nobody
 * able to fix it. ADMIN_PHONES is always honoured and cannot be revoked from
 * inside the app.
 */
export function bootstrapAdmins() {
  return new Set(
    (process.env.ADMIN_PHONES || '')
      .split(',').map((p) => normalisePhone(p)).filter(Boolean),
  );
}
...
export async function activeAdmins() {
  const admins = bootstrapAdmins();
  if (isMockMode()) {
    // In mock mode everyone allowlisted is an admin, so the panel is usable
    // locally without a database.
    for (const phone of allowedPhones().keys()) admins.add(phone);
    return admins;
  }
  try {
    const { rows } = await getPool().query(
      'SELECT phone FROM allowed_users WHERE active AND is_admin');
    rows.forEach((r) => admins.add(r.phone));
  } catch (err) {
    console.error('Could not read admins:', err.message);
  }
  return admins;
}

export const isAdminPhone = async (phone) => (await activeAdmins()).has(phone);

/**
 * A member's job when they are not an admin: 'caller' (the recovery board, as
 * before) or 'logistics' (orders and shipping only). Admins are 'admin'.
 */
export async function roleFor(phone) {
  if (await isAdminPhone(phone)) return 'admin';
  if (isMockMode()) return 'caller';
  try {
    const { rows } = await getPool().query(
      'SELECT role FROM allowed_users WHERE phone = $1 AND active', [phone]);
    return rows[0]?.role === 'logistics' ? 'logistics' : 'caller';
  } catch (err) {
    console.error('Could not read role:', err.message);
    return 'caller';
  }
}

/** What each role may reach. One place, so routes and the sidebar agree. */
export const canUseOrders = (role) => role === 'admin' || role === 'logistics';
export const canUseRecovery = (role) => role === 'admin' || role === 'caller';
