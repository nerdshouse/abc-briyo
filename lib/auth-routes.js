...
import {
  ...
  isAdminPhone, roleFor, canUseOrders, canUseRecovery,
} from './otp.js';
...
import { ensureSchema, recordLogin, touchLastSeen } from './db.js';
...
router.get('/me', async (req, res) => {
  ...
  res.json({
    ...
    ...(await (async () => {
      const role = session ? await roleFor(session.phone) : null;
      return { role, canOrders: canUseOrders(role), canRecovery: canUseRecovery(role) };
    })()),
    name: session ? await nameFor(session.phone) : null,
    driver: driver(),
...
async function membership(phone) {
  ...
  const role = allowed ? await roleFor(phone) : 'caller';
  const value = { allowed, admin: role === 'admin', role };
  membershipCache.set(phone, { value, expires: Date.now() + MEMBERSHIP_TTL_MS });

  // Piggy-backs on the cache miss, so presence costs one write per person per
  // minute rather than one per request.
  if (value.allowed) touchLastSeen(phone).catch(() => {});
  return value;
}
 ...
 */
/** Orders & logistics: admins and logistics staff. Callers are not included. */
export function requireOrders(req, res, next) {
  if (canUseOrders(req.session?.role)) return next();
  if (req.path.startsWith('/api/') || req.originalUrl.startsWith('/api/')) {
    return res.status(403).json({ ok: false, error: 'Orders access is not enabled for your account.' });
  }
  return res.redirect('/');
}

/** Recovery: admins and callers — everyone who had it before roles existed. */
export function requireRecovery(req, res, next) {
  if (canUseRecovery(req.session?.role)) return next();
  if (req.originalUrl.startsWith('/api/')) {
    return res.status(403).json({ ok: false, error: 'Recovery access is not enabled for your account.' });
  }
...
export async function requireAuth(req, res, next) {
  const session = verifySession(req.cookies?.[COOKIE]);
  if (session) {
    const { allowed, admin, role } = await membership(session.phone)
      .catch(() => ({ allowed: true, admin: false, role: 'caller' }));
    if (!allowed) {
    ...
    req.session = { ...session, isAdmin: admin, role };
    return next();
  }
  if (req.path.startsWith('/api/')) {
