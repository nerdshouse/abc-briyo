...
// Each signature is [offset, bytes]. WEBP is a RIFF container: "RIFF" at 0
// and "WEBP" at 8, with the file length in between.
const SIGNATURES = {
...
const EXTENSIONS = { pdf: 'application/pdf', png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', webp: 'image/webp' };

/** The formats every document type has always accepted. */
export const DEFAULT_FORMATS = ['pdf', 'png', 'jpg', 'jpeg'];
const formatList = (exts) => {
  const names = [...new Set(exts.map((e) => e.toUpperCase()))];
  return names.length > 1 ? `${names.slice(0, -1).join(', ')} and ${names.at(-1)}` : names[0];
};

/**
 * Returns { ok, mime, ext } or { ok:false, error } for an upload. `allowed`
 * narrows or widens the accepted extensions for a document type; the bytes
 * must match the extension either way, so a renamed file is refused.
 */
export function validateDocument(filename, buffer, allowed = DEFAULT_FORMATS) {
  const ext = String(filename || '').toLowerCase().split('.').pop();
  const mime = allowed.includes(ext) ? EXTENSIONS[ext] : null;
  if (!mime) return { ok: false, error: `Only ${formatList(allowed)} files can be uploaded here.` };
  ...
  const matches = SIGNATURES[mime].some((parts) => parts.every(([at, sig]) => sig.every((b, i) => buffer[at + i] === b)));
