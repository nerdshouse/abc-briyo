...
/**
 * Baked into every token so bumping SESSION_EPOCH invalidates all of them at
 * once. Sessions are stateless by design (no store, no per-request DB read),
 * which normally means a deactivated teammate keeps access until their token
 * expires. This is the emergency lever for that, at the cost of logging
 * everyone out — per-user revocation isn't worth a session table for this team.
 */
...
export function verifySession(token) {
  ...
  try {
    ...
    // Tokens issued before the current epoch are dead, even if unexpired.
    if (String(data.epoch ?? '1') !== epoch()) return null;
    return data;
  } catch {
    return null;
  }
