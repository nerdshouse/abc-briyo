...
const ENVELOPES = [
  ...
  // GoKwik's documented payload nests money under `totals` and the buyer under
...
export function normalizePayload(payload) {
  ...
  const cartId = pick(p, [
    // GoKwik-confirmed: `request_id` is the identifier on the abandoned-cart event.
    'request_id', 'requestId',
    'cart_id', 'cartId', 'checkout_id', 'checkoutId', 'id', 'token', 'cart_token',
    'cartToken', 'checkout_token', 'order_id', 'orderId', 'reference_id', 'moid', 'merchant_order_id',
  ]);
  ...
  const totalPrice = pick(p, [
    // GoKwik-confirmed: a `totals` object. Probed first, then legacy variants.
    'totals.total', 'totals.total_price', 'totals.grand_total', 'totals.amount',
    'totals.subtotal', 'totals.payable', 'totals.total_amount',
  ...
  const checkoutUrl = pick(p, [
    // GoKwik-confirmed: `abc_url` (abandoned basket checkout URL).
    'abc_url', 'abcUrl',
  ...
  // GoKwik-confirmed: `item_count` arrives as its own top-level field, so trust
  // it over counting the items array.
  const declaredItemCount = pick(p, ['item_count', 'itemCount', 'items_count', 'total_items', 'quantity']);
  ...
  return {
    ...
    itemCount:
      toNumber(declaredItemCount) ??
      (items ? items.reduce((sum, item) => sum + (toNumber(item?.quantity ?? item?.qty) ?? 1), 0) : null),
