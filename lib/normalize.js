...
/**
 * Envelopes that can carry a person's identity.
 *
 * Excludes `shipping`, which is the shipping *method* — GoKwik's block there has
 * `name: "Free Shipping"`, and the generic key `name` matched it, so every cart
 * showed a courier option where the customer should be. Same failure mode as
 * `shipping.price` winning the cart total.
 */
const IDENTITY_ENVELOPES = [
  'data', 'payload', 'cart', 'checkout', 'order',
  'customer', 'user', 'billing_address', 'shipping_address',
];
...
export function normalizePayload(payload) {
  ...
  const fullName = pick(p, [
    'customer_name', 'full_name', 'customer.name', 'customer.full_name',
    'user.name', 'billing_address.name', 'shipping_address.name',
    // Bare `name` last: it is the most collision-prone key in the payload.
    'name',
  ], IDENTITY_ENVELOPES);
  ...
  return {
    cartId: toStr(cartId),
    // Prefer the explicit first/last fields over a single `name` — they cannot
    // be confused with a shipping method or anything else generic.
    customerName: joinName(firstName, lastName) || joinName(fullName),
    phone: toStr(phone),
    email: toStr(email),
